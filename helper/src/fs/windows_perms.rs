...
use target_os_lib::core::{Error, Owned, Result, BOOL, PCWSTR, PWSTR};
use target_os_lib::Win32::Foundation::{E_UNEXPECTED, HANDLE, HLOCAL};
...
use target_os_lib::Win32::Security::{
    ...
    TokenUser, ACL, PSECURITY_DESCRIPTOR, SECURITY_ATTRIBUTES, TOKEN_ACCESS_MASK,
...
pub(super) fn apply(roots: &[&Path]) {
    ...
    // May fail for restricted processes (e.g. AppContainer);
    // the per-directory permissions set below still cover the common case.
    if let Err(e) = set_token_default_acl(&sd) {
        eprintln!("warning: could not set process token default ACL: {e}");
    }
...
/// Sets the default permissions applied to new files this process creates.
///
/// Covers files in directories not explicitly secured here, e.g. user-overridden
/// config paths.
fn set_token_default_acl(sd: &SecurityDescriptor) -> Result<()> {
    let mut present = BOOL::default();
    let mut pdacl: *mut ACL = ptr::null_mut();
    let mut defaulted = BOOL::default();
    // SAFETY: `sd.psd` is valid for the lifetime of `sd`; out-pointers are owned.
    unsafe {
        GetSecurityDescriptorDacl(sd.psd, &raw mut present, &raw mut pdacl, &raw mut defaulted)
    }?;
    if !present.as_bool() {
        return Err(Error::from_hresult(E_UNEXPECTED));
    }

    let token = open_process_token(TOKEN_ADJUST_DEFAULT)?;
    let tdd = TOKEN_DEFAULT_DACL { DefaultDacl: pdacl };
    // SAFETY: `tdd` is a local; `SetTokenInformation` copies the ACL internally
    // so freeing `sd` afterwards is safe.
    unsafe {
        SetTokenInformation(
            *token,
            TokenDefaultDacl,
            (&raw const tdd).cast(),
            u32::try_from(size_of::<TOKEN_DEFAULT_DACL>()).unwrap(),
        )
    }
}
...
fn create_private_directory(path: &Path, sa: &SECURITY_ATTRIBUTES) -> Result<()> {
    ...
    unsafe { CreateDirectoryW(PCWSTR(path_w.as_ptr()), Some(ptr::from_ref(sa))) }
}
